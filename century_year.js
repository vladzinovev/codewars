...
{
  if(year[2]==year[3] && year[3]==0){
    return `${Number(year.slice(0,2))}th`
  }
  let num=Number(year.slice(0,2))+1;
  if(num<20){
    return `${num}th`
  }
  num=String(num)
  switch(num[1]){
      case '1':num = `${num}st`; break;
      case '2':num = `${num}nd`; break;
      case '3':num = `${num}rd`; break;
      default:num = `${num}th`; break;
  }
  return num;
}
